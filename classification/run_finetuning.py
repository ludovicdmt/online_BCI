#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Collect data for to fine-tune a model. It uses data from previous sessions.
...
from pyacq_ext.datalistener import RealTimeDataListener
...
def calibration(
    ...
    Args:
        ...
        model_name2 (str): Name for the second stage of classification (MI).
    ...
    kernels = np.load(
        f"C:\\Users\\ludov\\Documents\\repos\\kernelsConstruction\\extra_kernels\\kernels_2014001.npy",
        allow_pickle=True,
    ).tolist()
    kernels = np.array(kernels)[[0, 1, 3, 6, 7]]  # [[2,3,5,6,7]]
    ...
    def on_new_chunk(label, new_chunk) -> None:
        ...
        """
        nonlocal trial_length, data_iti, calib_data, labels, kernels  # , asr
        # From uV to V
        # electrode selection
        ...
        data_iti.append(chunk_iti)  # ITI data to detect beta rebound
        # Save the calib data
        data_dict = {"calib_data": np.squeeze(np.array(calib_data)), "labels": labels}
        file_calib = "saved_models\\ft_data.pickle"
        ...
        file_iti = "saved_models\\iti_ft_data.pickle"
        ...
        if len(calib_data) == nb_epochs:
            ...
            # Store everything
            file = "saved_models\\refEA.pickle"
            ...
            # Load data from calibration to concatenate with the new data
            file = "saved_models\\rebound_data.pickle"
            with open(file, "rb") as f:
                data_dict = pickle.load(f)
            combined_epochs = np.concatenate(
                (combined_epochs, data_dict["calib_data"]), axis=0
            )
            combined_labels = np.concatenate(
                (combined_labels, data_dict["labels"]), axis=0
            )

            file = "saved_models\\calib_data.pickle"
            with open(file, "rb") as f:
                data_dict = pickle.load(f)
            calib_data = np.concatenate(
                (np.squeeze(np.array(calib_data)), data_dict["calib_data"]), axis=0
            )
            labels = np.concatenate((labels, data_dict["labels"]), axis=0)

            # Train a model to detect rebound
            model_name_M1 = model_name + "_stage1"
            grid = train_model(
            ...
            # Potatoes like threshold computation
            # Will be used during testing to avoid missclassification of MI trials
            # One-class MDM: if distance to MI barycenter too big, don't classify
            if model_name == "MDM":
            ...
            # Train the second stage model (MI left vs MI right) and save it
            model_name_M2 = model_name2 + "_stage2"
            grid = train_model(
                model_name=model_name_M2,
                X=calib_data,
                y=labels,
                sample_rate=sample_rate,
                kernels=kernels,
            ...
            file = f"saved_models\\{model_name_M2}.pickle"
            joblib.dump(grid, file)

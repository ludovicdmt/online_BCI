#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Fine-tune session with a single class. It uses data from previous sessions.
...
def calibration(
    ...
    def on_new_chunk(label, new_chunk) -> None:
        ...
        # Baseline removal
        chunk_MI -= np.mean(chunk_MI, axis=-1, keepdims=True)
        ...
        # Baseline removal
        chunk_iti -= np.mean(chunk_iti, axis=-1, keepdims=True)
        data_iti.append(chunk_iti)
        # Save the calib data
        data_dict = {"calib_data": np.squeeze(np.array(calib_data)), "labels": labels}
        file_calib = f"saved_models\\ft_data_{cmd}.pickle"
        ...
        file_iti = f"saved_models\\iti_ft_data_{cmd}.pickle"
        ...
        if len(calib_data) == nb_epochs:
            ...
            file = f"saved_models\\rebound_ft_data_{cmd}.pickle"
            with open(file, "wb") as f:
                pickle.dump(data_dict, f)
            data_dict = {
            ...
            file = f"saved_models\\calib_data_{cmd}.pickle"
            with open(file, "rb") as f:
                data_dict = pickle.load(f)
